...
# [이원화 적용] 게시판 지식을 'experience_base'로 동기화
...
st.markdown("""
    ...
    .tag-exp { background-color: #e0f2fe; color: #0369a1; } /* 경험 지식 태그 */
    .tag-man { background-color: #fef3c7; color: #92400e; } /* 매뉴얼 지식 태그 */
...
if mode == "🔍 통합 지식 검색":
    ...
    if user_q and (search_clicked or user_q):
        with st.spinner("경험과 매뉴얼 지식을 통합 분석 중..."):
            ...
            if combined_data:
                ...
                for d in combined_data:
                    # 데이터 출처에 따른 내용 구성
                    if 'solution' in d: # 경험 지식
                        context += f"[현장경험 / {d['manufacturer']} / {d['model_name']}]: {d['solution']}\n"
                    else: # 매뉴얼 지식
                        context += f"[매뉴얼원문 / {d['manufacturer']} / {d['model_name']}]: {d['content']}\n"
                
                ans_p = f"수질 전문가 답변. 현장 경험 우선 참고. 에러코드 정확히 전달. 3줄 요약.\n데이터: {context} \n 질문: {user_q}"
                st.info(ai_model.generate_content(ans_p).text)
                st.markdown("---")
                for d in combined_data:
                    is_exp = 'solution' in d
                    tag_cls = "tag-exp" if is_exp else "tag-man"
                    with st.expander(f"[{'현장경험' if is_exp else '매뉴얼'}] {d['manufacturer']} | {d['model_name']}"):
                        st.markdown(f'<span class="source-tag {tag_cls}">{d["registered_by"] if is_exp else d.get("file_name", "매뉴얼")}</span>', unsafe_allow_html=True)
                        st.write(d['solution'] if is_exp else d['content'])
            else: st.warning("⚠️ 일치하는 지식을 찾지 못했습니다.")

# --- 2. 현장 노하우 등록 (experience_base 저장) ---
elif mode == "📝 현장 노하우 등록":
...
# --- 3. 문서(매뉴얼) 등록 (manual_base 저장) ---
    ...
    st.warning("💡 대용량 PDF는 순차적으로 정밀 저장됩니다.")
    ...
    if up_file and st.button("🚀 매뉴얼 저장 시작"):
        try:
            ...
            meta_p = f"제조사와 모델명 추출(JSON): {all_text[:3000]}"
            ...
            for i, chunk in enumerate(chunks):
                ...
                status_text.text(f"📥 저장 중... ({i+1}/{len(chunks)})")
            
            st.success(f"✅ {success_count}개의 매뉴얼 지식이 등록되었습니다!"); st.rerun()
...
# --- 4. 데이터 전체 관리 (이원화 관리) ---
    ...
    with tab1:
        m_search = st.text_input("🔍 경험 지식 검색", key="exp_search")
        if m_search:
            ...
            for row in res.data:
                with st.expander(f"{row['manufacturer']} | {row['issue']}"):
                    st.write(row['solution'])
                    if st.button("🗑️ 삭제", key=f"e_{row['id']}"): supabase.table("experience_base").delete().eq("id", row['id']).execute(); st.rerun()
    with tab2:
        d_search = st.text_input("🔍 매뉴얼 검색", key="man_search")
        if d_search:
            res = supabase.table("manual_base").select("*").or_(f"manufacturer.ilike.%{d_search}%,content.ilike.%{d_search}%").execute()
            for row in res.data:
                with st.expander(f"{row['manufacturer']} | {row['model_name']} (일부)"):
                    st.write(row['content'][:300])
                    if st.button("🗑️ 삭제", key=f"m_{row['id']}"): supabase.table("manual_base").delete().eq("id", row['id']).execute(); st.rerun()
        ...
        if q_res.data:
            ...
            with st.form("ans_f", clear_on_submit=True):
                ...
                if st.form_submit_button("✅ 답변 등록") and a_auth and a_cont:
                    ...
                    sync_qa_to_experience(q['id']) # 경험 지식으로 동기화
                    st.rerun()
    else:
        st.subheader("💬 질문 게시판")

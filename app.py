...
import ui_community
import ui_inventory 
import ui_collab  # [필수] 협업 기능(연락처/일정) 연결
...
# [UI] 공통 레이아웃 설정
# --------------------------------------------------------------------------
st.set_page_config(page_title="금강수계 AI V265", layout="wide", initial_sidebar_state="collapsed")
st.markdown("""<style>
...
</style><div class="fixed-header">🌊 금강수계 수질자동측정망 AI V265 (통합 관리 시스템)</div>""", unsafe_allow_html=True)

# --------------------------------------------------------------------------
# [메뉴] 라우팅 처리 (드롭다운 방식 복구)
# --------------------------------------------------------------------------
_, menu_col, _ = st.columns([1, 2, 1])
with menu_col:
    # [Rollback] 드롭다운 메뉴 + "협업" 메뉴 추가

...
def semantic_split_v122(text, target_size=900, min_size=250):
...
# AI 분류 엔진 (V122 표준 반영)
def v122_classify_data(content):
    try:
        prompt = f"""너는 금강수계 수질관리 전문가야. 데이터를 아래 체계에 맞춰 JSON으로 분류해.
        1. 도메인: [기술지식, 행정절차, 복지생활]
        2. 세부분류 및 항목:
           - 측정기기: TOC, TN, TP, 일반항목, VOCs, 물벼룩, 황산화, 미생물, 발광박테리아 등
           - 채수시설: 펌프, 레듀샤, 호스, 커플링, 캡록, 여과 필터 등
        데이터: {content}
        응답: {{"domain": "값", "sub_category": "값", "item": "값"}}"""
        res = ai_model.generate_content(prompt)
        return extract_json(res.text)
    except: return None

# --- UI 설정 ---
st.set_page_config(page_title="금강수계 AI 챗봇 V122", layout="centered", initial_sidebar_state="collapsed")
...
st.markdown("""
    ...
    <div class="fixed-header"><span class="header-title">🌊 금강수계 수질자동측정망 AI 챗봇 V122</span></div>
        ...
        if user_q:
            with st.spinner("전문 지식을 필터링 중..."):
                ...
                if final_pool:
                    ...
                    st.info(ai_model.generate_content(f"질문: {user_q} 데이터: {final_pool[:10]}").text)
                    for i, d in enumerate(final_pool[:10]):
                        with st.expander(f"{i+1}. [{d.get('sub_category')}] {str(d.get('issue') or '상세내용')[:40]}..."):
...
# --- 4. 데이터 전체 관리 (V122: 연동형 수동 분류 UI) ---
    ...
    with tabs[3]: # 수동 분류실
        ...
        if not review_list: st.success("🎉 모든 데이터가 정돈되었습니다!")
        else:
            for item in review_list:
                with st.container():
                    st.markdown(f"**[데이터 원문]**\n{item.get('content') or item.get('solution')}")
                    with st.form(key=f"rv_v122_{t_name}_{item['id']}"):
                        c1, c2, c3 = st.columns(3)
                        # 1. 도메인 선택
                        m_dom = c1.selectbox("도메인", list(DOMAIN_MAP.keys()), key=f"d_{item['id']}")
                        # 2. 세부분류 선택 (도메인에 따라 변경)
                        sub_cats = list(DOMAIN_MAP[m_dom].keys()) + ["직합 입력"]
                        m_sub = c2.selectbox("세부분류", sub_cats, key=f"s_{item['id']}")
                        # 3. 상세 항목 선택 (세부분류에 따라 연동)
                        items_list = DOMAIN_MAP[m_dom].get(m_sub, ["공통"]) if m_sub != "직접 입력" else ["공통"]
                        m_item = c3.selectbox("상세 항목/부품", items_list + ["직접 입력"], key=f"i_{item['id']}")
                        
                        m_manual_sub = c2.text_input("└ 세부분류 직접 입력") if m_sub == "직접 입력" else ""
                        m_manual_itm = c3.text_input("└ 항목 직접 입력") if m_item == "직접 입력" else ""
                        
                        if st.form_submit_button("✅ 분류 확정"):
                            f_sub = m_manual_sub if m_sub == "직접 입력" else m_sub
                            f_itm = m_manual_itm if m_item == "직접 입력" else m_item
    ...
    with tabs[4]: # 지식 재건축 (V121 로직 유지)
        st.subheader("🏗️ 매뉴얼 지식 재건축")
        files = list(set([r['file_name'] for r in supabase.table("manual_base").select("file_name").execute().data if r.get('file_name')]))
        t_file = st.selectbox("재구성 파일", options=files)
        if st.button("🚀 맥락 복원 시작"):
            ...
            if old_rows:
                ...
                new_chunks = semantic_split_v122(full_text)
    ...
    with tabs[2]: # 시맨틱 최신화
        if st.button("🚀 정밀 분류 실행 (20건)"):
            rows = supabase.table("knowledge_base").select("*").eq("semantic_version", 0).limit(20).execute().data
            for r in rows:
                res = v122_classify_data(r.get('solution') or r.get('content') or "")
                if res:
                    supabase.table("knowledge_base").update({
                        "domain": res.get('domain'), "sub_category": res.get('sub_category'),
                        "measurement_item": res.get('item'), "semantic_version": 1
                    }).eq("id", r['id']).execute()
            st.rerun()

# --- 2, 3, 5 메뉴 (로직 유지) ---
elif st.session_state.page_mode == "📝 지식 등록":
    with st.form("reg_v122"):
    ...
    if up_f:
        ...
        if st.button("🚀 학습 시작"):
            ...
            chunks = semantic_split_v122(all_t)
            for chunk in chunks:

...
# --- UI 스타일 (시인성 강화) ---
st.set_page_config(page_title="금강수계 AI V142", layout="wide", initial_sidebar_state="collapsed")
st.markdown("""<style>
    ...
    .meta-bar { background-color: rgba(128, 128, 128, 0.15); border-left: 5px solid #004a99; padding: 8px; border-radius: 4px; font-size: 0.8rem; margin-bottom: 10px; color: inherit; }
    .guide-box { background-color: #f8fafc; border: 1px solid #e2e8f0; padding: 12px; border-radius: 8px; font-size: 0.85rem; color: #1e293b; margin-bottom: 15px; }
    ...
    .stForm { background-color: rgba(255, 255, 255, 0.05); padding: 10px; border-radius: 8px; }
</style><div class="fixed-header">🌊 금강수계 수질자동측정망 AI V142</div>""", unsafe_allow_html=True)
...
if mode == "🔍 통합 지식 검색":
    ...
    with main_col:
        ...
        # [V142] 임계값 설명 가이드 복구
        st.markdown(f"""<div class="guide-box">🎯 <b>검색 모드: {"정밀 타격" if u_threshold > 0.6 else "균형"}</b><br>값이 높을수록 질문과 단어가 정확히 일치하는 정보만 검색합니다.</div>""", unsafe_allow_html=True)
    ...
    if user_q and (search_btn or user_q):
        with st.spinner("의도를 분석하고 장비 정보를 정밀 필터링 중..."):
                ...
                if final:
                    ...
                    for d in final[:8]:
                        ...
                        with st.expander(f"[{d.get('model_name','공통')}] 상세 내용 {v_mark}"):
                            ...
                            # [V142 핵심] 현장 라벨 수정 폼 (상호작용성 강화)
                            with st.form(key=f"edit_{d['u_key']}"):
                                st.markdown("🔧 **장비 정보 교정 (품질 개선용)**")
...
# --- 4. 데이터 전체 관리 (APIError 원천 봉쇄) ---
    ...
    with tab_col:
        ...
        with tabs[3]: 
            ...
            t_sel = st.radio("대상 테이블", ["경험", "매뉴얼"], horizontal=True, key="apprv_target")
            t_name = "knowledge_base" if t_sel == "경험" else "manual_base"
            
            # [V142 수정] 매뉴얼 테이블인 경우에만 일괄 승인 UI 노출 및 쿼리 실행
            if t_name == "manual_base":
                st.markdown('<div style="background-color:rgba(0, 74, 153, 0.1); padding:15px; border-radius:10px;">', unsafe_allow_html=True)
                st.write("📂 **매뉴얼 파일 단위 일괄 승인**")
                ...
                if files:
                    ...
                    target_f = c1.selectbox("일괄 승인할 파일 선택", files, label_visibility="collapsed")
                    if c2.button("🚀 선택 파일 전체 승인", use_container_width=True):
                        db.bulk_approve_file(t_name, target_f); st.toast("일괄 승인 완료!"); time.sleep(1); st.rerun()
            ...
            else: st.success("🎉 모든 검토가 완료되었습니다.")

# --- 3. 문서 등록 및 2. 지식 등록 (기능 유지) ---
elif mode == "📄 문서(매뉴얼) 등록":
    ...
    with up_col:
        ...
        if up_f and st.button("🚀 심층 분석 학습 시작", use_container_width=True):
            ...
            chunks = semantic_split_v142(all_t)
            for chunk in chunks:

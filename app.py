...
# [V119] 의미 중심 텍스트 분할 함수
def semantic_split(text, target_size=800):
    # 마침표, 물음표, 느낌표 뒤에 공백이 오는 패턴으로 문장 분리
...
def v119_classify_data(content):
    try:
...
st.markdown("""
    ...
    <div class="fixed-header"><span class="header-title">🌊 금강수계 수질자동측정망 AI 챗봇 V119</span></div>
...
if st.session_state.page_mode == "🔍 통합 지식 검색":
    ...
    if user_q and (search_clicked or user_q):
        with st.spinner("지식을 필터링 중..."):
            try:
                target_domain = "복지생활" if "생활정보" in search_mode else v119_route_intent(user_q)
...
# --- 4. 데이터 전체 관리 ---
    ...
    tabs = st.tabs(["📊 로그 분석", "📝 경험 리파이너", "📄 매뉴얼 리파이너", "🚫 교정 기록", "🧹 시맨틱 최신화", "🚨 수동 분류 대기실"])
    
    with tabs[4]: # 시맨틱 최신화
        st.subheader("🧹 데이터베이스 시맨틱 최신화")
        t_table = st.radio("대상", ["경험 지식", "매뉴얼 지식"], horizontal=True, key="m_target")
        t_name = "knowledge_base" if t_table == "경험 지식" else "manual_base"
        unlabeled = supabase.table(t_name).select("id", count="exact").eq("semantic_version", 0).execute()
        st.metric("대기 데이터", f"{unlabeled.count or 0} 건")
        
        if st.button("🚀 최신화 시작"):
            rows = supabase.table(t_name).select("*").eq("semantic_version", 0).limit(20).execute().data
            if rows:
                with st.status("🏗️ 분석 중...", expanded=True) as status:
                    for r in rows:
                        try:
                            content = r.get('solution') or r.get('content') or ""
                            result = v119_classify_data(content[:2500])
                            if result:
                                ...
                                st.write(f"✅ ID {r['id']} 완료")
                            else:
                                supabase.table(t_name).update({"semantic_version": 1, "review_required": True}).eq("id", r['id']).execute()
                        except:
                            supabase.table(t_name).update({"semantic_version": 1, "review_required": True}).eq("id", r['id']).execute()
                    status.update(label="완료!", state="complete")
                st.rerun()

    with tabs[5]: # 수동 분류 대기실
        ...
        # [V119] 하이브리드 드롭다운용 기존 카테고리 추출
        existing_data = supabase.table(t_name).select("sub_category").execute().data
        all_cats = sorted(list(set([r['sub_category'] for r in existing_data if r.get('sub_category')])))
        cat_options = all_cats + ["직접 입력"]
            ...
            for item in review_list:
                with st.container():
                    ...
                    with st.form(key=f"rv_form_{t_name}_{item['id']}"):
                        ...
                        # [V119 핵심] 드롭다운 + 직접 입력 UI
                        m_sub_sel = c2.selectbox("세부분류 선택", options=cat_options, key=f"sel_{item['id']}")
                        m_sub_manual = ""
                        if m_sub_sel == "직접 입력":
                            m_sub_manual = c2.text_input("└ 직접 입력하기", key=f"inp_{item['id']}")
                        ...
                        if st.form_submit_button("✅ 분류 확정"):
                            ...
                            if final_sub:
                                supabase.table(t_name).update({
                                    "domain": m_dom, "sub_category": final_sub, 
                                    "measurement_item": m_item, "review_required": False
                                }).eq("id", item['id']).execute()
                                st.success("승격 완료!"); time.sleep(0.5); st.rerun()
                st.divider()

# --- 2. 지식 등록 ---
elif st.session_state.page_mode == "📝 지식 등록":
    st.subheader("📝 신규 지식 등록")
    with st.form("reg_v119", clear_on_submit=True):
...
# --- 3. 문서 등록 (V119: 의미론적 분할 적용) ---
    ...
    if up_f:
        ...
        if st.button("🚀 지식 학습 시작"):
            ...
            # [V119 핵심] 문장 단위 분할 적용
            chunks = semantic_split(all_t, target_size=800)
            
            p_bar = st.progress(0)
            for i, chunk in enumerate(chunks):
                supabase.table("manual_base").insert({
                    "domain": f_dom, "content": clean_text_for_db(chunk), 
                    "file_name": up_f.name, "embedding": get_embedding(chunk), 
                    "semantic_version": 1
                }).execute()
                p_bar.progress((i+1)/len(chunks))
            st.success(f"총 {len(chunks)}개의 의미 덩어리로 학습 완료!"); st.rerun()

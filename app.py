...
def analyze_query_v96(text):
    ...
    life_keys = ["맛집", "식당", "카페", "추천", "주차", "메뉴", "점심", "저녁", "회식"]
...
def sync_qa_to_knowledge(q_id):
    try:
        ...
        is_t, is_l, mfr, mod = analyze_query_v96(q_d['title'] + q_d['content'])
...
# [V96 핵심] 모든 플랫폼 아이콘 및 버튼 강제 삭제 CSS
st.markdown("""
    <style>
    /* 상단 헤더 및 배포 버튼 완전 제거 */
    ...
    .fixed-header {
        ...
        padding: 12px 0; z-index: 999; text-align: center; box-shadow: 0 4px 10px rgba(0,0,0,0.2);
    }
    .header-title { font-size: 1.15rem; font-weight: 800; letter-spacing: 1px; }
...
selected_mode = st.selectbox("☰ 메뉴 선택", options=menu_options, index=menu_options.index(st.session_state.page_mode), label_visibility="collapsed")
if selected_mode != st.session_state.page_mode:
    st.session_state.page_mode = selected_mode
    st.rerun()

# --- 1. 통합 지식 검색 (V96: 클린 UI 및 하이브리드 검색) ---
if st.session_state.page_mode == "🔍 통합 지식 검색":
    ...
    if user_q and (search_clicked or user_q):
        with st.spinner("전문 지식을 분석하고 있습니다..."):
            try:
                is_tech_q, is_life_q, target_mfr, target_mod_num = analyze_query_v96(user_q)
                ...
                if final_pool:
                    ...
                    ans_p = f"""금강수계 전문가입니다. 질문: {user_q} \n 데이터: {context} \n
                    제공된 데이터를 요약하되 문장 끝에 [{display_tag('출처태그')}]를 표기하세요."""
                    
                    st.subheader("🤖 AI 정밀 요약 답변")
                    st.info(ai_model.generate_content(ans_p).text)
                    
                    st.markdown('<div class="guide-box">✅ 하단 리스트에서 각 정보가 정확한지 알려주세요. <b>[무관함]</b> 사유를 적어주시면 AI가 더 똑똑해집니다.</div>', unsafe_allow_html=True)
                    ...
                    for d in final_pool[:10]:
                        ...
                        with st.expander(f"[{d_tag}] [상황] {str(d.get('issue') or '상세 지식')[:35]}..."):
                            ...
                            if c1.button("👍 도움됨", key=f"v_ok_{s_tag}", use_container_width=True):
                                if update_single_helpfulness(s_tag): st.success("추천 완료!"); time.sleep(0.5); st.rerun()
                            with c2:
                                with st.popover("❌ 무관함", use_container_width=True):
                                    r_sel = st.selectbox("제외 사유", ["브랜드 불일치", "주제 무관", "오래된 정보", "기타"], key=f"rs_{s_tag}")
                                    c_in = st.text_input("상세 의견", key=f"cm_{s_tag}")
                                    if st.button("확인", key=f"cf_{s_tag}"):
                                        if add_to_blacklist(user_q, s_tag, r_sel, c_in): st.error("제외되었습니다."); time.sleep(0.5); st.rerun()
                else:
                    st.warning("⚠️ 지식을 찾지 못했습니다. 미해결 과제로 등록되었습니다.")
                    log_unsolved(user_q, "검색결과 없음", is_life)
            except Exception as e: st.error(f"조회 실패 (V96): {e}")

# --- 2. 현장 노하우 등록 ---
elif st.session_state.page_mode == "📝 현장 노하우 등록":
    st.subheader("📝 현장 노하우 등록")
    cat_sel = st.selectbox("분류", ["기기점검", "현장꿀팁", "맛집/정보"])
    with st.form("reg_v96", clear_on_submit=True):
        if cat_sel != "맛집/정보":
            c1, c2 = st.columns(2)
            m_sel = c1.selectbox("제조사", ["시마즈", "코비", "백년기술", "케이엔알", "YSI", "직접 입력"])
            m_man = c1.text_input("└ 제조사 직접 입력")
...
# --- 4. 데이터 전체 관리 ---
    ...
    with t2:
        ms = st.text_input("🔍 경험 검색 (키워드/제조사)")
        if ms:
    ...
    with t3:
        st.subheader("📂 파일 관리")
        ds = st.text_input("🔍 파일명 검색", placeholder="비워두면 최근 순")
        ...
        if res_m.data:
            ...
            for f in unique_f:
                with st.expander(f"📄 {f}"):
                    if st.button("🗑️ 매뉴얼 전체 삭제", key=f"df_{f}"): supabase.table("manual_base").delete().eq("file_name", f).execute(); st.rerun()
    with t4:
        ...
        if bl:
            ...
            st.dataframe(df_bl.rename(columns={'query': '검색어', 'source_id': 'ID', 'reason': '제외사유', 'comment': '의견'})[['검색어', 'ID', '제외사유', '의견']], use_container_width=True)

# --- 5. 질문 게시판 ---
elif st.session_state.page_mode == "💬 질문 게시판 (Q&A)":
    if st.session_state.get('selected_q_id'):
        if st.button("⬅️ 질문 목록으로"): st.session_state.selected_q_id = None; st.rerun()
        ...
        for a in ans_d: st.markdown(f'<div style="background:#f8fafc; padding:12px; border-radius:8px; margin-bottom:5px; border-left:3px solid #004a99;"><b>{a["author"]}</b>: {a["content"]}</div>', unsafe_allow_html=True)
        with st.form("ans_v96"):
            at, ct = st.text_input("작성자 성함"), st.text_area("답변 및 조치 방법")
        ...
        st.subheader("💬 질문 게시판")
        with st.popover("➕ 새로운 질문 남기기"):
            with st.form("q_v96"):
                cat, auth, tit, cont = st.selectbox("항목", ["기기이상", "일반"]), st.text_input("작성자"), st.text_input("질문 제목"), st.text_area("상세 상황 설명")
...
# --- 6. 미해결 과제 ---
    ...
    st.subheader("🆘 동료의 도움이 필요한 현장 상황")
    unsolved = supabase.table("unsolved_questions").select("*").eq("status", "대기중").order("created_at", desc=True).execute().data
    if not unsolved: st.success("✨ 모든 문제가 해결되었습니다!")
    else:
        for item in unsolved:
            with st.container():
                st.markdown(f"<div style='background:#fff1f2; padding:15px; border-radius:10px; border-left:5px solid #b91c1c; margin-bottom:10px;'><b>{item['query']}</b><br><small>등록 사유: {item['reason']}</small></div>", unsafe_allow_html=True)
                with st.form(key=f"form_sos_{item['id']}"):
                    ans_in = st.text_area("조치 노하우 입력")
                    ...
                    if st.form_submit_button("✅ 지식으로 저장"):
                        if ans_in:

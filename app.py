        ...
        with st.spinner("3트랙 정밀 분석 중..."):
            try:
                ...
                if final_results:
                    ...
                    ans_p = f"수질 전문가 답변. 3줄 요약. 데이터: {context} \n 질문: {user_q}"
...
# --- 2. 현장 노하우 등록 (분류별 UI 분기 최적화) ---
elif st.session_state.page_mode == "📝 현장 노하우 등록":
    st.subheader("📝 현장 노하우 등록")
    cat_choice = st.selectbox("등록할 정보 분류", ["기기점검", "현장꿀팁", "맛집/정보"])
    
    with st.form("exp_reg_form", clear_on_submit=True):
        if cat_choice in ["기기점검", "현장꿀팁"]:
            c1, c2 = st.columns(2)
            with c1:
                mfr_sel = st.selectbox("제조사", ["시마즈", "코비", "백년기술", "케이엔알", "YSI", "직접 입력"])
                mfr_manual = st.text_input("└ 직접 입력 시")
            with c2:
                model_name = st.text_input("모델명 (예: TOC-4200)")
                item_name = st.text_input("측정항목 (예: TOC)")
            reg_user = st.text_input("등록자 성함")
            issue_title = st.text_input("현상 (제목)")
            solution_desc = st.text_area("조치 내용 (상세)")
        else:
            # 맛집/정보 전용 UI [cite: 2025-12-22]
            st.info("🍴 주변 맛집이나 유용한 생활 정보를 공유해주세요!")
            c1, c2 = st.columns(2)
            res_name = c1.text_input("식당/장소 이름", placeholder="예: 옥천 삼동소바")
            res_loc = c2.text_input("위치 (지역/주소)", placeholder="예: 충북 옥천군")
            res_menu = st.text_input("주요 메뉴 (분류)", placeholder="예: 메밀소바, 돈까스")
            reg_user = st.text_input("등록자 성함")
            issue_title = st.text_input("정보 요약 (제목)", placeholder="예: 해장하기 좋은 국밥집")
            solution_desc = st.text_area("상세 설명 (내용)", placeholder="맛, 가격대, 주차 정보 등을 적어주세요.")

        if st.form_submit_button("✅ 지식 저장"):
            if cat_choice in ["기기점검", "현장꿀팁"]:
                final_mfr = mfr_manual if mfr_sel == "직접 입력" else mfr_sel
                final_model, final_item = model_name, item_name
            else:
                final_mfr, final_model, final_item = res_name, res_loc, res_menu
            
            if final_mfr and issue_title and solution_desc:
                supabase.table("knowledge_base").insert({
                    "category": cat_choice, "manufacturer": clean_text_for_db(final_mfr),
                    "model_name": clean_text_for_db(final_model), "measurement_item": clean_text_for_db(final_item),
                    "issue": clean_text_for_db(issue_title), "solution": clean_text_for_db(solution_desc),
                    "registered_by": clean_text_for_db(reg_user),
                    "embedding": get_embedding(f"{cat_choice} {final_mfr} {final_model} {issue_title} {solution_desc}")
                }).execute()
                st.success("🎉 등록되었습니다!")
    ...
    if up_file:
        if 's_m' not in st.session_state or st.session_state.get('l_f') != up_file.name:
            with st.spinner("분석 중..."):
                ...
                info_res = extract_json(ai_model.generate_content(f"제조사/모델명 JSON: {preview[:3000]}").text) or {}

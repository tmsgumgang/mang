...
# --- UI 스타일 ---
st.set_page_config(page_title="금강수계 AI V141", layout="wide", initial_sidebar_state="collapsed")
st.markdown("""<style>
...
</style><div class="fixed-header">🌊 금강수계 수질자동측정망 AI V141</div>""", unsafe_allow_html=True)
...
if mode == "🔍 통합 지식 검색":
    ...
    with main_col:
        ...
        st.markdown(f'<div class="guide-box">🎯 <b>정밀 타격(0.6↑)</b>: 정확한 용어 매칭 / <b>포괄 탐색(0.4↓)</b>: 의미 위주 검색</div>', unsafe_allow_html=True)
    ...
    if user_q and (search_btn or user_q):
        with st.spinner("의도를 분석하고 장비 정보를 필터링 중..."):
...
# --- 4. 데이터 전체 관리 (에러 수정 완료) ---
    ...
    with tab_col:
        ...
        with tabs[3]: # 라벨 승인
            ...
            t_sel = st.radio("대상", ["경험", "매뉴얼"], horizontal=True, key="apprv_target")
            t_name = "knowledge_base" if t_sel == "경험" else "manual_base"
            
            # [V141 에러 수정] 컬럼 존재 여부에 따른 조건부 UI
            if t_name == "manual_base":
                st.markdown('<div style="background-color:#f0f9ff; padding:15px; border-radius:10px;">', unsafe_allow_html=True)
                ...
                if files:
                    ...
                    target_f = c1.selectbox("파일 선택", files)
...
# --- 3. 문서 등록 및 2. 지식 등록 (기능 유지) ---
    ...
    with up_col:
        ...
        if up_f and st.button("🚀 심층 분석 학습 시작", use_container_width=True):
            ...
            chunks = semantic_split_v141(all_t)
            for chunk in chunks:
    ...
    with reg_col:
        with st.form("reg_v141"):
            f_iss = st.text_input("제목")
            f_sol = st.text_area("조치방법")

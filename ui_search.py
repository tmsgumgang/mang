...
from utils_search import perform_unified_search # [V179] 통합 검색 엔진 호출

def show_search_ui(ai_model, db):
    # [V175/179] 가독성 최적화 CSS (3줄 요약 박스 디자인 강화)
    st.markdown("""<style>
        .summary-box { 
            ...
            with res_col:
                ...
                for d in final[:6]:
                    ...
                    with st.expander(f"[{d.get('measurement_item','-')}] {d.get('model_name','공통')} (신뢰도: {score}%) {v_mark}"):
                        ...
                        if c1.button("✅ 질문과 연관있음", key=f"v179_up_{d['u_key']}"):
                            if db.save_relevance_feedback(user_q, d['id'], t_name, 1):
                                st.success("반영됨!"); time.sleep(0.5); st.rerun()
                        if c2.button("❌ 질문과 무관함", key=f"v179_down_{d['u_key']}"):
                            if db.save_relevance_feedback(user_q, d['id'], t_name, -1):
                        ...
                        st.markdown("---")
                        # 데이터 교정 폼 (V160 유지)
                        with st.form(key=f"edit_v179_{d['u_key']}"):
